...
func (d *decoder) Property() (*Props, error) {
	size, err := d.Int()
	if err != nil {
		return nil, err
	} else if size == 0 {
	...
	for {
		...
		case PayloadFormatIndicator:
			if prop.PayloadFormatIndicator, err = dd.Uint8(); err != nil {
				return nil, err
			}
		...
		case MaximumQoS:
			if prop.MaximumQoS, err = dd.Uint8(); err != nil {
				return nil, err
			}
		...
		case SubscrptionIdentifierAvailable:
			if i, err := dd.Int(); err != nil {
				...
				prop.SubscriptionIdentifierAvailable = i > 0
			}
		case SharedSubscriptionsAvaliable:
			if i, err := dd.Int(); err != nil {
				return nil, err
			} else {
				prop.SharedSubscriptionsAvailable = i > 0
			}
		}
	}
	return prop, nil
